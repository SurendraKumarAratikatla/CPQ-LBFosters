class CostAPI:
    ...
    def accessMatCostTbl(self):
        ...
        qitemplant = self.Product.ParseString('<*CTX( Quote.CurrentItem.CustomField(LBF_QU_PLANT1))*>')
        
        if data:
            # if str(self.marketId) != "8" and str(self.marketId) != "7":
            try:
                currentPlant = self.Product.Attr('Plant (Own or External)').GetValue()
            except Exception as e:
                currentPlant = qitemplant
    ...
    def getRolledupItemId(self):
        ...
        currUpdateLineItem = self.Product.ParseString('<*CTX( Quote.CurrentItem.RolledUpItemNumber )*>')
        # if int(currUpdateLineItem) == 1 and int(self.currentItemId) > 1:
        self.currentItemId = int(currUpdateLineItem)
    ...
    def update_partners(self):
        ...
        prospect = context.Quote.GetCustomField("LBF_CF_Prospect").Value
        prospect_data = SqlHelper.GetFirst("select S4Customer,SOrg from LBF_QU_PROSPECTSUB where SOrg='{0}'".format(self.salesOrg))
        if prospect_data:
            prospect_value = str(prospect_data.S4Customer)
        else:
            prospect_value = "" # we will check this later for this value if required

        if prospect == "Yes":
            for party in involved_parties:
                self.payload['ORDER_PARTNERS'][party.PartnerFunctionKey] = {
                    'PARTN_ROLE': party.PartnerFunctionKey,
                    'PARTN_NUMB': prospect_value
                }
        else:
            for party in involved_parties:
                self.payload['ORDER_PARTNERS'][party.PartnerFunctionKey] = {
    ...
    def UOMQtyUpdate(self):
        for item in context.Quote.GetAllItems():
            ...
            unitOfMesasure = str(item.AsMainItem.UnitOfMeasure)
            if str(int(item.RolledUpQuoteItem)) == str(int(self.currentItemId)) and str(item['LBF_QU_TARUOM']) != 'FT':
                ...
                try:
                    ...
                    self.payload['ORDER_ITEMS_IN']['TARGET_QTY'] = str(float(item.Quantity))
                    self.payload['ORDER_SCHEDULE_IN']['REQ_QTY'] = str(float(item.Quantity))
...
#configOpen= Product.ExternalConfiguration
isConfigOpen = context.Quote.GetCustomField("LBF_CF_InConfigScreen").Value

if vcpPro and proIsComplete and isConfigOpen:
