class CostAPI:
    ...
    def responseUpdateAtCart(self,resMatCost,resErrorMsg):
        # Error msg
        Trace.Write('resErrorMsg---'+str(resErrorMsg))
        ...
        for item in context.Quote.GetAllItems():
            if str(int(item.RolledUpQuoteItem)) == str(int(self.currentItemId)):
                Trace.Write('COST API Response COST elif-->'+str(resMatCost))
                Trace.Write('self.errMsg-->'+str(self.errMsg))
                try:
...
proIsComplete = Product.IsComplete
