def ShipmentCheck():
    ...
    market_update_table = QuoteHelper.Get(str(quoteNumber)).QuoteTables['Market_Notes']
    check1 = False
    check2 = False
    for row in market_update_table.Rows:
        Trace.Write(row['Note_Type'])
        if row['Note_Type'] == 'Shipment':
            check1 = True
            if row['Notes']:
                check2 = True
    if (check1 == False) or (check1 == True and check2 == False):
        context.Quote.GetCustomField('LBF_CF_SHIPPING').Value = 'No'
        Trace.Write('if')
        Trace.Write('check1'+str(check1))
        Trace.Write('check2'+str(check2))
    else:
        context.Quote.GetCustomField('LBF_CF_SHIPPING').Value = ''
        Trace.Write('else')
        Trace.Write('check1'+str(check1))
        Trace.Write('check2'+str(check2))
    Trace.Write(context.Quote.GetCustomField('LBF_CF_SHIPPING').Value)
