...
class MultiSalesOrg:
    ...
    def placeOrder(self):
        ...
        if len(incomplete_products) == 0:
            ...
            for item in data:
				# removing items that are either in order created status or order pending
                for cust in item["CustomFields"]:
                    if cust["Name"] == "LBF_QU_SO_STATUS" and cust["Content"] in ["Order Confirmation Pending", "Order Created"]:
                        data.Remove(item)            
