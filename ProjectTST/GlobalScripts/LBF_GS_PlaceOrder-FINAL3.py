...
class MultiSalesOrg:
    ...
    def placeOrder(self):
        ...
        if len(incomplete_products) == 0:
            ...
            Log.Info("quoteheaderData---->"+str(quoteheaderData))
            Log.Info("quoteheaderData['CustomFields']11111111111---->"+str(quoteheaderData["CustomFields"]))
            customFields = quoteheaderData["CustomFields"]
            Log.Info("customFields---->"+str(customFields))
            updatedcustomFields = self.updatePaymentTerms(customFields)
            Log.Info("updatedcustomFields---->"+str(updatedcustomFields))
            quoteheaderData["CustomFields"] = updatedcustomFields
            Log.Info("quoteheaderData['CustomFields']22222222222---->"+str(quoteheaderData["CustomFields"]))
            ...
            if self.inv_party_check() != None:
                ...
                for sales_org, items in sales_org_groups.items():
                    ...
                    for index, item in enumerate(items, start=1):
                        ...
                        rail_value = None
                        rail_uspr_cont = None
                        rail_uspr_value = None
                        rail_rent_cont = None
                        rail_rent_value = None
                            ...
                            if query_condmap_data:
                                for row in query_condmap_data:
                                    if str(row.CustomField) == "LBF_QU_MAT_CST_OVR":
                                        ...
                                        if field["Content"] and float(field["Content"]) > 0: # to handle decimal values but one test case will fail where user enter negative value will see this later.
                                        ...
                                        if field["Content"] == "QT" and str(row.Variation) == "QT":
                                            ...
                                            hand_cont = str(row.ConditionType) 
                                        elif field["Content"] == "MAN" and str(row.Variation) == "MAN": #error  433
                                            hand_cont = str(row.ConditionType)                                             
                                    ...
                                    elif str(row.CustomField) == "LBF_QU_RAIL_USPR":
                                        if field["Content"]:
                                            rail_uspr_cont = str(row.ConditionType)
                                            rail_uspr_value = field["Content"]
                                    elif str(row.CustomField) == "LBF_QU_RAIL_TRNRNT":
                                        if field["Content"]:
                                            rail_rent_cont = str(row.ConditionType)
                                            rail_rent_value = field["Content"]                                            
                        sell_dict = {
                        ...
                        rail_uspr = {
                                     "ITM_NUMBER": item_Number,
                                     "COND_TYPE" : rail_uspr_cont,
                                     "COND_VALUE": rail_uspr_value
                        }
                        rail_rent = {
                                     "ITM_NUMBER": item_Number,
                                     "COND_TYPE" : rail_rent_cont,
                                     "COND_VALUE": rail_rent_value
                        }
                        if str(marketCode) != "3131":
                            w_dat.append(sell_dict)
                            w_dat.append(rail_uspr)
                            w_dat.append(rail_rent)
                            vcpPro = SqlHelper.GetList("Select * from PRODUCTS where PRODUCT_CATALOG_CODE ='{0}' and IsSyncedFromBackOffice ='{1}' and IsSimple='{2}'".format(partNumber,'True','False'))
                            if len(vcpPro) == 0:
                                w_dat.append(mat_dict)
                            # validation for Handling cost # might plant validation can be remove if we recive the soluction from S/4 side
