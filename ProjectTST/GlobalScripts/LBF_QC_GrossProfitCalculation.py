...
for item in items:
    try:
        ...
        Trace.Write("item['LBF_QU_GP_PER']--->"+str(float(calculate_gross_profit_percent(selling_price, total_cost))))

        #item['LBF_QU_SELPRICE'] = str(round(selling_price * quantity, 2))
        Trace.Write("item['LBF_QU_SELPRICE']--->"+str(round(selling_price * quantity, 2)))
        gross_profit_value = (selling_price - total_cost) * quantity
        #item['LBF_QU_GP_VAL'] = str(round(gross_profit_value, 2))
        Trace.Write("item['LBF_QU_GP_VAL']--->"+str(round(gross_profit_value, 2)))

        #Log.Write(f"Item {item.ProductId}: GP% = {profit}, Total Sell Price = {item['LBF_QU_SELPRICE']}")
        Log.Info('LBF_QC_GrossProfitCalculation In Try'+ str(item.PartNumber))
    except Exception as e:
        #Log.Write(f"Error on item {item.ProductId}: {str(e)}")
        Trace.Write('LBF_QC_GrossProfitCalculation In Except'+ str(item.PartNumber))
        #item['LBF_QU_GP_PER'] = 0.0
        #item['LBF_QU_SELPRICE'] = "0.00"
        Trace.Write('LBF_QC_GrossProfitCalculation Error....in '+str(e))