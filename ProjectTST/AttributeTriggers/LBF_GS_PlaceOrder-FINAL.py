                ...
                for sales_org, items in sales_org_groups.items():
                    ...
                    for index, item in enumerate(items, start=1):
                        ...
                        for field in item['CustomFields']:
                            ...
                            # if field["Name"] == "LBF_QU_MAT_CST_OVR":
                            #     if field["Content"] and int(float(field["Content"])) !=0:
                            #         mat_cont = "ZPRS"
                            #         mat_value = field["Content"]
                            ...
                            #     if field["Content"] == "QT":
                            #         frieght_cont = "ZF01"
                            #     elif field["Content"] == "WT":
                            #         frieght_cont = "ZF00"
                            #     elif field["Content"] == "FC" or field["Content"] == "FIX":
                            #         frieght_cont = "ZF02"
                            ...
                            #         hand_cont = "ZS01"
                            #         hand_value = field["Content"]
                            # elif field["Name"] == "LBF_QU_SELPRICE_UN" and listPriceCheck != "Simple": #(field["Name"] == "LBF_QU_CXT_SMPVCP" and field["Content"] != "Simple")
                            #     if field["Content"]:
                            #         sell_cont = "ZMAN"
                            #         sell_value = field["Content"]
                        sell_dict = {
                    ...
                    # Log.Info("requestData before json"+str(updated_items))
                    for qtItem in updated_items['CustomFields']:
                        # Log.Info("qtItem"+str(qtItem))
                        if qtItem['Name'] == 'LBF_QU_ITEMDESC':
                            Log.Info("qtItem['Name']"+str(qtItem['Name']))
                            qtItem['Content'] = str(qtItem['Content']).replace('“', '"').replace('”', '"').replace('’', "'").replace('‘', "'")
                        elif qtItem['CustomFields']['Name'] == 'LBF_QU_Notes':
                            qtItem['Content'] = str(qtItem['Content']).replace('“', '"').replace('”', '"').replace('’', "'").replace('‘', "'")

                    requestData = {
                        'Root': {
                            "Quotes": quoteheaderData,
                            "Quoteitems": updated_items
                        }
                    }
                    Log.Info("requestData json"+str(requestData))
                    json_obj = JsonHelper.Serialize(requestData)
                    Log.Info("formated json"+str(json_obj))
