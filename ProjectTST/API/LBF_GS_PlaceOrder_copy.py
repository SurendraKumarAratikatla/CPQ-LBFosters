...
class MultiSalesOrg:
    def __init__(self):
        ...
        self.ItemStack = {}
        self.Root = {}
        self.spMsg = "Please Enter Sold-to Party"
        self.shMsg = "Please Enter Ship-to Party"
        self.spshMsg = "Please enter Sold-to and Ship-to in Customer Info Tab"
    ...
    def deleteDupQuoteMsgs(self):
        ...
        for msg in self.quoteInfo.Messages:
            if str(msg.Content) != str(self.spMsg):
                Trace.Write(msg.Content)
                spcount += 1
                #self.quoteInfo.AddMessage(str(self.spMsg), MessageLevel.Error, False)
                return spcount
            elif str(msg.Content) != str(self.shMsg):
                Trace.Write(msg.Content)
                shcount += 1
                #self.quoteInfo.AddMessage(str(self.shMsg), MessageLevel.Error, False)
                return shcount
            elif str(msg.Content) != str(self.spshMsg):
                Trace.Write(msg.Content)
                spshcount += 1
                #self.quoteInfo.AddMessage(str(self.spshMsg), MessageLevel.Error, False)
                return spshcount
        return 0
