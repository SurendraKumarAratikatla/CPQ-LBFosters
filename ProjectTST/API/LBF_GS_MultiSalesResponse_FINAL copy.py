def update_MultiSalesQuote(data):
    ...
    for obj in data:
        ...
        for item in quote.GetAllItems():
            ...
            Log.Info("str(int(item_No // 10))-->"+str(int(item_No) // 10))
            if str(item.RolledUpQuoteItem) == str(int(item_No) // 10):
            ...
            if str(item['LBF_QU_SO_STATUS']) != "Order Created":
                count += 1
        quote.Save()
        ...
        if count == 0:
            ...
            context.Quote.ChangeStatus('Order Placed')
    return res
